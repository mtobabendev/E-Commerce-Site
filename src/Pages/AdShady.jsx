...
import ShadyAd from '../assets/AdShady.json';
...
// ---------------------------------------------------------------------------------------
    ...
    const adslides = ShadyAd.ShadyAd.map((item, i) => {
        return {name: ShadyAd.ShadyAd[i].name, img: ShadyAd.ShadyAd[i].image, title: ShadyAd.ShadyAd[i].title}});

    const [adslideState, setAdSlideState] = useState(adslides);
...
// ----------------------------------------------------------------------------------------
    ...
    const [prevAdSlide, setPrevAdSlide] = useState();
    ...
    useEffect(() => {
        ...
        interval = setInterval(() => {
            setAdSlideIndex((prev) => {
                setPrevAdSlide(prev);
                return (prev + 1) % adslides.length; // the % slides.length wraps it back around to the beginning when it's at the end
            })
        }, 3000);
    ...
    function plusAdSlides(n) {
        setPrevAdSlide(adslideIndex);
        const newIndex = (adslideIndex + n + adslides.length) % adslides.length;
        setAdSlideIndex(newIndex);
    }
    
    // from https://www.w3schools.com/howto/howto_js_slideshow.asp
    return (
            <div className='ad-main'>
                <div className='ad-slideshow-container'>
                    {adslideState.map((item, index) => (
                    <>
                    <a className='arrow prev' onClick={() => { adplusSlides(-1); setArrowStop (true)}}>&#10094;</a>
                    <NavLink to='/products' state={{ name: item.title }} className={({ isActive }) => (isActive ? 'active' : '')}>
                        <div
                            key={index}
                            className={`ad-slide ${index === adslideIndex ? 'active' : index === prevAdSlide ? 'previous' : ''}`}
                            style={{ backgroundImage: `url(${item.img})`}}
                            title={item.title}
                        >
                            <h3 className='ad-text'>{item.name}</h3>
                        </div>
                    </NavLink>
                    <a className='arrow next' onClick={() => { plusAdSlides(1); setArrowStop (true)}}>&#10095;</a>
                    </>
                ))}
                </div>
            </div>
